...
if (process.platform !== "darwin" || process.arch !== "arm64") {
  console.warn(
    "[mlx-ts] skipping mlx-host install (supported only on darwin/arm64)."
  );
  process.exit(0);
}
...
const BIN_DIR = path.resolve(
  new URL("../bin/darwin-arm64", import.meta.url).pathname
);
...
async function main() {
  console.log(
    `[mlx-ts] installing mlx-host for ${process.platform}/${process.arch}`
  );
  console.log(`[mlx-ts] download base: ${BASE}`);
  console.log(`[mlx-ts] install dir: ${BIN_DIR}`);
